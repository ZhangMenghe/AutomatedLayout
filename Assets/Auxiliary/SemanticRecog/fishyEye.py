...
def undistort(img_path, DIM, K, D, balance=0.0, dim2=None, dim3=None):
    # img = cv2.imread(img_path)
    data =np.load(img_path)
    img = np.zeros([data.shape[0], data.shape[1], data.shape[2]])
...
if __name__ == '__main__':
    ...
    np.save('D', D)
    # for p in sys.argv[1:]:
    undistort("calibrate_img/test/infrared4.npy")
